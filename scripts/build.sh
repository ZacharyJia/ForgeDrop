...
OUT_CTL_BIN="${ROOT_DIR}/bin/forgedrop-ctl"

usage() {
  cat <<'EOF'
Usage: scripts/build.sh [options]

Builds forge-drop binaries and the embedded web UI.

Options:
  --skip-web       Skip building web UI (web/dist)
  --skip-go        Skip building Go backend
  --install        Run npm ci (or npm install) before web build
  --out <path>     Output path for forge-drop binary (default: bin/forge-drop)
  --out-ctl <path> Output path for forgedrop-ctl binary (default: bin/forgedrop-ctl)
...
Examples:
  ...
  scripts/build.sh --out-ctl /tmp/forgedrop-ctl
EOF
}
...
if [[ $SKIP_GO -eq 0 ]]; then
  ...
  mkdir -p "$(dirname "$OUT_CTL_BIN")"
  go build -o "$OUT_BIN" ./cmd/forge-drop
  go build -o "$OUT_CTL_BIN" ./cmd/forgedrop-ctl
  echo "built: $OUT_BIN"
  echo "built: $OUT_CTL_BIN"
fi
